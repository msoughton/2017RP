...
void SupernovaAna::analyze(art::Event const & e)
{
  ...
  // Get the Hits out of the event
  art::Handle< std::vector< cheat::BackTracker > > backtracker_list;
  e.getByLabel(fBackTrackerLabel, backtracker_list);

 for(unsigned int i = 0; i < backtracker_list->size();i++){
    cheat::BackTracker const& backtracker = backtracker_list->at(i);
    
    std::vector< sim::IDE > ides;
    try{
    backtracker.HitToSimIDEs( chit, ides );
    }
    catch(...){};  
  // std::vector< art::Ptr<recob::Hit > > hits_list,std::vector
 };
